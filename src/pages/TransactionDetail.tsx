...
import {
  ArrowLeft, ArrowLeftRight, Flag, FileText,
  ExternalLink, ChevronDown, ZoomIn, AlertTriangle,
} from "lucide-react";
...
const TransactionDetail = () => {
  ...
  const [localNotes, setLocalNotes] = useState<{ by: string; note: string; time: string }[]>([]);
  ...
  const noteMutation = useMutation({
    ...
    onSuccess: () => {
      setLocalNotes((p) => [...p, { by: "You", note: newNote.trim(), time: "Just now" }]);
      setNewNote("");
      setNoteOpen(false);
  ...
  if (!tx) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3">
        ...
        <button onClick={() => navigate("/transactions")} className="text-[13px] text-orange-500 hover:underline">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr]">
          ...
          <div className="p-5 border-b lg:border-b-0 lg:border-r border-gray-100/80 dark:border-gray-700/20 space-y-5">
            ...
            <div className="flex flex-col items-center text-center gap-2">
              ...
              <div>
                <p className="text-[15px] font-bold text-gray-900 dark:text-white">{tx.user_full_name ?? "—"}</p>
                      ...
                      <div key={i} className="bg-white/60 dark:bg-[#1C1C1C]/60 rounded-[10px] p-2.5">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[11px] font-semibold text-orange-600 dark:text-orange-400">
                            {n.by ?? n.admin_name ?? "Admin"}
                          </span>
                          <span className="text-[10px] text-gray-400">{n.time ?? n.created_at ?? ""}</span>
                        </div>
                        <p className="text-[11px] text-gray-700 dark:text-gray-300">{n.note ?? n.content}</p>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="p-5">
            ...
            <div className="space-y-0 border border-gray-200/50 dark:border-gray-700/30 rounded-[12px] overflow-hidden mb-4">
              {[
                ...
                <div
                  key={label}
                  className={cn("flex items-center justify-between px-4 py-2.5", i % 2 === 0 ? "bg-[#F5F5F5]/30 dark:bg-[#2D2B2B]/30" : "")}
                >
                  <span className="text-[12px] text-gray-500 dark:text-gray-400">{label}</span>
                  {badge ? (
                    <Badge className={cn("border rounded-full text-[11px] font-semibold px-3 py-0 h-5 bg-transparent", statusColor)}>
            ...
            {tx.details && (
              <div className="border border-gray-200/50 dark:border-gray-700/30 rounded-[12px] p-4">
                <p className="text-[12px] font-semibold text-gray-900 dark:text-white mb-2">Additional Details</p>
  ...
  <DialogContent className="bg-white dark:bg-[#1C1C1C] border-gray-200/50 dark:border-gray-700/30 rounded-[20px] shadow-2xl max-w-md">
    ...
    <div className="space-y-3 py-1">
      ...
      <div className="bg-[#F5F5F5]/80 dark:bg-[#2D2B2B]/80 rounded-[12px] p-3 space-y-1.5">
        {[
          { label: "Transaction ID", value: (tx.id ?? tx.transaction_id ?? id ?? "").toString().slice(0, 8).toUpperCase() },
          { label: "User", value: tx.user_full_name ?? "—" },
